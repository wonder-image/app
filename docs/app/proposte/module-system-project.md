...
{% hint style="warning" %}
🚧 **Proposta non (ancora) implementata.** Questo è un documento di progetto
architetturale. Diverse parti (lifecycle persistente, install/uninstall, event
bus) **non sono nel codice**. Per ciò che è realmente implementato vedi
[Moduli](../concetti/moduli/README.md). Non considerare nulla qui come garantito
senza verificarlo nel codice.
{% endhint %}
