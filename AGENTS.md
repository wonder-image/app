...
- Scheduled work lives in `class/App/Scheduler`: extend `AbstractTask` or use
  ...
  an HTTP endpoint in a task. CLI-only `bin/scheduler.php` is generated by
  build/update; API triggers require the existing `@system` Bearer identity
  (the generated site file alone is gitignored; framework `bin/` is source)
  and enqueue only. Reuse `NamedLock`, preserve saved schedules on sync, and
  retain execution logs uniformly for 180 days. Sitemap wraps the existing
