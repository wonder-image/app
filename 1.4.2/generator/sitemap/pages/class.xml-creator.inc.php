<?php // This file is protected by copyright law and provided under license. Reverse engineering of this file is strictly prohibited.
																									...
																									function zGRTxSUQWeBFkNDq($urls_completed, &$t9v8ohOpFm)
																														...
																														$xz = 'rss';
																														if($this->SVH4BzIyh56a_HEGqt['xs_rssinfo'])
																														if(!$this->SVH4BzIyh56a_HEGqt['xs_rssinfo_max'] ||
																														($this->rssno < $this->SVH4BzIyh56a_HEGqt['xs_rssinfo_max']))
																														{
																														$OTgW3XUe40DYXayyQ = ($this->PgnYRF1yCnq0blPgv3&&!isset($this->PgnYRF1yCnq0blPgv3[$cu['link']]))
																														|| $this->urls_prevrss[$cu['link']]
																														|| !$this->SVH4BzIyh56a_HEGqt['xs_rssage'];
																														if($this->SVH4BzIyh56a_HEGqt['xs_rssincmask'])
																														if(!preg_match('#'.str_replace(' ', '|', preg_quote($this->SVH4BzIyh56a_HEGqt['xs_rssincmask'],'#')).'#',$cu['link']))
																														$OTgW3XUe40DYXayyQ = false;
																														if($OTgW3XUe40DYXayyQ)
																														{
																														$KpnOpjQSejtign8 = $this->urls_prevrss[$cu['link']] ? strtotime($this->urls_prevrss[$cu['link']]) : time();
																														$KpnOpjQSejtign8 = gmdate('D, d M Y H:i:s +0000', $KpnOpjQSejtign8);
																														$this->rssno++;
																														$_rsst = $t2?$t2:preg_replace('#^.*/#', '', $l);
																														o1ts5NHkE20c($uPF8pmxRDuN9zs,
																														$this->BVvxOQnEYIQ8b->MZz43evFukacB7v($S9tIHGBcxfbNnwARatY[2],
																														array(
																														'URL'=>$l,
																														'GUID' => md5($l),
																														'TITLE'=>$_rsst,
